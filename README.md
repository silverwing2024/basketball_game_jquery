...
- computer 객체 내에 score, percent2, percent3 속성을 대입했고 이는 초기 ver의 comScore, comPercent2, comPercent3을 리팩토링한 사례에 해당함

동적 속성 접근: computer['percent' + shootType] 방식으로 속성에 동적으로 접근
- computer.percent2 -> computer['percent'+shootType]로 변경함

AI 로직 추가: 점수 차이에 따라 컴퓨터의 슛 성공 확률이 조정되도록 구현함. 유저가 큰 점수차로 리드하면 컴퓨터의 득점확률을 높이며 반대로 컴퓨터가 큰 점수차로 리드를 하게 되면 유저의 득점 확률을 높였다. 점수차이가 7점 이상이면 2점 슛의 확률을 50%에서 60%로, 3점 슛의 확률을 33%에서 38%로 상향시키며 11점 이상이면 2점슛과 3점슛의 확률을 각각 70%,43%로 상향시키도록 구현하였다. 
...
또한 게임의 진행을 컴퓨터 턴일 때는 사용자가 버튼을 누르는 것이 아니라 

시간의 흐름에 따라 자동적으로 득점이 일어나도록 변경하여 

실제로 컴퓨터와 대전을 하고 있다는 느낌이 들도록 기능 수정을 할 필요 있음.

현재는 컴퓨터 턴일 때도 사용자가 버튼을 누르는 행위를 통해 컴퓨터의 득점을 결정하게 되므로 

유저의 입장에서는 1인 2역의 역할극이라는 느낌이 들게됨.
